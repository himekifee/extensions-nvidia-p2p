...
The stock `ghcr.io/siderolabs/installer-base:v1.12.6` won't work because its kernel was compiled with a different signing key than the local kernel build tree. This script produces an installer-base whose embedded kernel trusts the same key that signed the custom `.ko` files.
...
- **`make target-nvidia-modules-p2p-lts-pkg`** and **`make target-nvidia-modules-p2p-lts`** handle the bldr-based module compilation and sysext packaging.
