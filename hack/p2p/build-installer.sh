...
for extra_system_extension_ref in "${extra_system_extension_refs[@]}"; do
  case "$extra_system_extension_ref" in
    "$custom_sysext_ref"|"$toolkit_ref")
      continue
      ;;
    "$official_open_module_ref"|*nvidia-open-gpu-kernel-modules-lts*)
...
# Append IOMMU kernel args to the extracted cmdline (idempotent).
existing_cmdline="$(tr -d '\0' < "$verify_dir/cmdline.bin")"
for iommu_arg in intel_iommu=on iommu=pt; do
  case " $existing_cmdline " in
    *" $iommu_arg "*) ;;
    *) existing_cmdline="$existing_cmdline $iommu_arg" ;;
  esac
done
printf '%s' "$existing_cmdline" > "$verify_dir/cmdline.bin"
...
toolkit_name = "nvidia-container-toolkit-lts"
glibc_name = "glibc"
...
pattern = re.compile(
    r"name: (nvidia-modules-p2p-lts|nvidia-container-toolkit-lts|nvidia-open-gpu-kernel-modules-lts)"
)
...
if ! printf '%s\n' "$extension_names" | grep -F "name: nvidia-modules-p2p-lts" >/dev/null; then
  fail "custom P2P NVIDIA sysext is missing from the installer initrd"
fi
printf '%s\n' "$extension_names" | grep -F "name: nvidia-container-toolkit-lts" >/dev/null || fail "stock NVIDIA userspace toolkit sysext is missing from the installer initrd"

if printf '%s\n' "$extension_names" | grep -F "name: nvidia-open-gpu-kernel-modules-lts" >/dev/null; then
  fail "official nvidia-open-gpu-kernel-modules-lts was included alongside the custom P2P sysext"
fi

printf 'verified installer image: %s\n' "$installer_digest_ref"
printf 'verified installer digest: %s\n' "$installer_digest"
printf 'verified included extensions:\n%s\n' "$extension_names"
printf 'verified excluded extension: nvidia-open-gpu-kernel-modules-lts\n'
