...
if selected_track != "lts":
    fail(f"mismatch: nvidia.selectedTrack '{selected_track}' != 'lts'")
...
upstream_repo_url="${matrix_values[0]}"
upstream_release="${matrix_values[1]}"
upstream_commit="${matrix_values[2]}"
donor_branch="${matrix_values[3]}"
donor_base_release="${matrix_values[4]}"
donor_base_commit="${matrix_values[5]}"
donor_patch_commit="${matrix_values[6]}"
donor_branch_head_commit="${matrix_values[7]}"
diff_url="${matrix_values[8]}"
...
printf 'validated patch apply against NVIDIA LTS tag %s\n' "$upstream_release"
